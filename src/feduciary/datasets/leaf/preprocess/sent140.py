...
def preprocess(root):
    ...
    logger.info(f'[DATA LOAD] [LEAF - {DATASET_NAME.upper()}] Process GloVe embeddings (300 dim.)...!')
    glove_vocab = _get_glove_vocab(path)
    logger.info(f'[DATA LOAD] [LEAF - {DATASET_NAME.upper()}] ...finished processing GloVe embeddings!')
